		...
		if key[0..5] == 'filter'
			@order.each do |item|
				if key[6..5+item.length] == item
					filters[item] = [] if filters[item] == nil
					string = key[6+item.length..-1]
					string += value unless value == "" or value == nil
					filters[item] += [string]
				end
			end
		end
...
def filter_list request
	...
	@order2.each do |item|
		...
		if @schema[item] == "LSTRING"
			...
			unless filters[item].nil?
				filters[item].each do |x|
					text += "<td><a href='#{query = request.query.clone ; query.delete "filter#{item}#{x}" ; query.delete "filter#{item}" ; build_query(query)}'>#{x}</a></td>"
				end
			end
